...
public class CartWithTwoPackagedItems {
	...
	public static void classSetUp() throws Exception {
		...
		// create another packaged item
		try {
			secondUPC = new UPC("222222222222");
		} catch (InvalidUPCException e) {
			fail("Invalid UPC");
		}
		secondWeight = 1.23;
	...
	public void setUp() throws Exception {
		...
		gi2 = firstSCO.addItem(secondUPC);
	}
	...
	public void calculatingTotalCost() {
		...
		assertEquals(firstPDB.lookUpItem(firstUPC).getPrice() + firstPDB.lookUpItem(secondUPC).getPrice(), total, 0.0);
	}
	...
	public void payingForGroceries() {
		...
		gi = coc.listItems().nextElement();
		assertEquals(secondUPC.toString(), gi.getInfo().getCode().toString());
		
		assertEquals(secondWeight, gi.getWeight(), EPSILON);
		
		assertEquals(firstPDB.lookUpItem(secondUPC).getPrice(), gi.getPrice(), 0.0);
